...
function draw (width, height, bounds, data) {
  ...
  data = data.features;
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        getEveryCoord(coordArray,context,scale,bounds);
    }
  // Loop over the features…
//  for (var i = 0; i < data.length; i++) {
//    var coordArray = data[i].geometry.coordinates;
//      //only 1 level
//      if(coordArray.length == 1 && $.isNumeric(coordArray[0][0][0])){
//          for(var j in coordArray[0]){
//              var coord = coordArray[0][j];
//              
//              longitude = coord[0];
//              latitude = coord[1];
...
//      }
//      //2 level
//      else if(coordArray[0].length == 1 && $.isNumeric(coordArray[0][0][0][0])){
//          for(var k in coordArray){
//              for(var j in coordArray[k][0]){
//                  var coord = coordArray[k][0][j];
//              
//                  longitude = coord[0];
//                  latitude = coord[1];
//                  point = mercator(longitude, latitude);
...
//      }
    ...
    workset.appendChild(canvas);
}

function getEveryCoord(coordinateArr,context,scale,bounds){
    //not coordinateArr
    if(! $.isNumeric(coordinateArr[0][0])){
        for(var i in coordinateArr){
            getEveryCoord(coordinateArr[i],context,scale,bounds);
        }
    }
    else{
        ...
        for(var j in coordinateArr){
            ...
            var point = mercator(longitude, latitude);
            console.log(point);
            // Scale the points of the coordinate
            // to fit inside our bounding box
//            console.log(bounds.xMin+'///'+scale);
            point.x = (point.x - bounds.xMin);
            point.y = (bounds.yMax - point.y);
            console.log(point);
//            console.log(isNaN(point.x));
            console.log(bounds.xMin);
            console.log(isNaN(bounds.xMin));
            console.log(bounds.yMax);
            console.log(isNaN(bounds.yMax));
//            console.log(isNaN(Number(scale)));
            // If this is the first coordinate in a shape, start a new path
            if (j === 0) {
                context.context.beginPath();
                context.moveTo(point.x, point.y);
